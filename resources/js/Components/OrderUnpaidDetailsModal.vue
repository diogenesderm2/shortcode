    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-[92vw] max-w-4xl mt-10 p-6">
      ...
      <div class="flex items-center justify-between mb-3">
        <div>
          <h2 class="text-xl font-bold text-gray-900">
            Pedido #{{ order.number }}
            <span class="ml-2 bg-blue-500 text-white text-xs font-semibold px-2 py-1 rounded-full">Pendente</span>
          </h2>
          <p class="text-sm text-gray-500">Detalhes do pedido ainda não pago</p>
        </div>
      ...
      <div class="mt-6">
        ...
        <div class="flex items-center justify-between max-w-md">
          ...
          <div class="font-semibold text-blue-600">{{ formatCurrency(order.total) }}</div>
        </div>
        <div class="flex items-center justify-between max-w-md mt-2">
          <div class="text-gray-700">Valor Pago</div>
          <div class="font-semibold text-teal-500">{{ formatCurrency(order.valorPago ?? 0) }}</div>
        </div>
      </div>
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="text-gray-700">
              ...
              <div class="text-sm">Aguardando pagamento</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <table class="min-w-full text-sm">
            ...
            <tbody>
              ...
              <tr class="border-t">
                ...
                <td class="py-2 px-2 text-right font-bold text-blue-600">{{ formatCurrency(order.total) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
